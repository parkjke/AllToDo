    ...
    class Coordinator: NSObject, NMFMapViewTouchDelegate, NMFMapViewCameraDelegate {
        ...
        func renderRawItems(mapView: NMFMapView, allItems: [UnifiedMapItem]) {
            ...
            for item in allItems {
                ...
                // 2. Icon Type (High Performance Cache)
                let (baseName, _, _) = UnifiedMapItem.resolveClusterStyle(items: [item])
                
                // Naver Scale: Reduced to 28x35 (Same as Kakao)
                let targetSize = CGSize(width: 28, height: 35)
                if let i = PinImageHelper.shared.fetchBasePin(named: baseName, targetSize: targetSize) {
        ...
        func renderWasmResults(mapView: NMFMapView, clusterResult: [Int32], allItems: [UnifiedMapItem]) {
            ...
            for (idx, items) in clusters.enumerated() {
                ...
                // [FIX] Naver Scale: Reduced to 28x35
                let targetSize = CGSize(width: 28, height: 35)
                if let basePin = PinImageHelper.shared.fetchBasePin(named: baseName, targetSize: targetSize) {
                    if items.count == 1 {
                        marker.iconImage = NMFOverlayImage(image: basePin)
                    } else {
                        let finalImage = PinImageHelper.shared.applyBadge(to: basePin, count: items.count)
