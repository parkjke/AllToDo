            ...
            // Buckets matching
             ...
             for (idx, items) in clusters.enumerated() {
                 ...
                 if items.count == 1, let item = items.first {
                     ...
                     // Icon (High Performance Cache)
                     let (baseName, _, _) = UnifiedMapItem.resolveClusterStyle(items: [item])
                     // [FIX] Standard size 40x50 (1.0x) as per MAP_PIN_DESIGN.md
                     marker.icon = PinImageHelper.shared.fetchBasePin(named: baseName, targetSize: CGSize(width: 40, height: 50))
                 } else {
                     // Cluster Logic
                     
                     // [FIX] Adjust Anchor for Cluster (Right Badge Overhang)
                     // Tip is at (24, 54) of 48x58. 24/48 = 0.5. 
                     // GroundAnchor is ratio of image size.
                     marker.groundAnchor = CGPoint(x: 0.5, y: 1.0)
                                          
                      // [FIX] Centralized Logic
                      let (baseName, _, _) = UnifiedMapItem.resolveClusterStyle(items: items)
                                          // 1. Fetch cached base Pin
                      // [FIX] Standard size 40x50 (1.0x) as per MAP_PIN_DESIGN.md
                      if let basePin = PinImageHelper.shared.fetchBasePin(named: baseName, targetSize: CGSize(width: 40, height: 50)) {
                         // 2. Combine with dynamic badge
                         marker.icon = PinImageHelper.shared.applyBadge(to: basePin, count: items.count)
                     }
                 }
