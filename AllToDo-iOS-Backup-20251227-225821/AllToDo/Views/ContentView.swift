...
struct ContentView: View {
    ...
    private func updateMapItems() {
        ...
        // [FIX] Relax filter: Show all items that have a valid coordinate, regardless of path recording
        let items = allItems.filter { item in
            guard let loc = item.location else { return false }
            let itemDate = item.begin_time ?? item.date_time ?? Date(timeIntervalSince1970: Double(item.created_at)/1000.0)
