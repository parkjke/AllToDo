...
struct ApplePathMapView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
        ...
        func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
             ...
             // [FIX] Use History Pin Icon (Prioritize Asset via Helper)
             if let img = PinImageHelper.shared.fetchBasePin(named: "PinHistory") {
                 view?.image = img
             } else {
                 view?.image = UIImage(systemName: "clock.fill")!.withTintColor(.red)
             }
    ...
    func updateUIView(_ uiView: GMSMapView, context: Context) {
        ...
        if let img = PinImageHelper.shared.fetchBasePin(named: "PinHistory") {
            start.icon = img
             ...
             start.icon = UIImage(systemName: "clock.fill")?.withTintColor(.red)
        }
        start.map = uiView
        ...
        if let img = PinImageHelper.shared.fetchBasePin(named: "PinHistory") {
             ...
             end.icon = UIImage(systemName: "clock.fill")?.withTintColor(.red)
        }
        end.map = uiView
...
struct KakaoPathMapView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, MapControllerDelegate {
        ...
        var registeredShapeStyleIDs = Set<String>()
        ...
        private func renderPath(mapView: KakaoMap) {
            ...
            // [FIX] Increase zOrder to 15000 (above tiles) to ensure visibility
            let layer = manager.getShapeLayer(layerID: "pathLayer") ?? manager.addShapeLayer(layerID: "pathLayer", zOrder: 15000)
            ...
            if !registeredShapeStyleIDs.contains(styleID) {
                let style = PolylineStyle(styles: [
                    // [FIX] Map width directly or with small scaling (1.5x)
                    PerLevelPolylineStyle(bodyColor: uiColor, bodyWidth: UInt(selectedWidth * 1.5), strokeColor: UIColor.clear, strokeWidth: 0, level: 0)
                ])
                manager.addPolylineStyleSet(PolylineStyleSet(styleSetID: styleID, styles: [style]))
                registeredShapeStyleIDs.insert(styleID)
            }
        ...
        func addPins(mapView: KakaoMap, coordinates: [CLLocationCoordinate2D]) {
            ...
            let layer = manager.getLabelLayer(layerID: "pathPins") ?? manager.addLabelLayer(option: LabelLayerOptions(layerID: "pathPins", competitionType: .none, competitionUnit: .poi, orderType: .rank, zOrder: 2000))
            
            // [FIX] Kakao Scale: 0.7x (28x35) to match main map look
            let targetSize = CGSize(width: 28, height: 35)
            let baseName = "PinHistory"
            let color = UIColor.red
            let styleID = "style_PathPin_History"
            
            if !registeredStyleIDs.contains(styleID) {
                var finalImage: UIImage?
                // [FIX] Prioritize Asset via Custom Helper (28x35 for Kakao)
                if let img = PinImageHelper.shared.fetchBasePin(named: baseName, targetSize: targetSize) {
                    finalImage = img
                }
                
                if let img = finalImage {
                    // [FIX] Precision Anchor: 14/36 (~0.388), 1.0
                    let anchorX = 14.0 / 36.0
                    let anchor = CGPoint(x: anchorX, y: 1.0)
                    manager.addPoiStyle(PoiStyle(styleID: styleID, styles: [PerLevelPoiStyle(iconStyle: PoiIconStyle(symbol: img, anchorPoint: anchor), level: 0)]))
