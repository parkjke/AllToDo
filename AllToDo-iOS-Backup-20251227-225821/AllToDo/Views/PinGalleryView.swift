...
struct PinGalleryView: View {
    ...
    func generatePin(name: String, count: Int) -> UIImage? {
        // [FIX] Use high-performance cache and modular assembly
        let baseSize = CGSize(width: 40, height: 50)
        if let basePin = PinImageHelper.shared.fetchBasePin(named: name, targetSize: baseSize) {
            if count <= 1 {
                return basePin
            } else {
                return PinImageHelper.shared.applyBadge(to: basePin, count: count)
            }
        }
        return nil
    }
