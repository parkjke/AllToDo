...
class AppLocationManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private var isProcessingBuffer = false // [NEW] Race Condition Guard
    ...
    private func processBuffer() async {
        ...
        if compressedInts.count % 2 == 0 && !compressedInts.isEmpty {
            ...
            // [FALLBACK] If WASM failed or returned no points, use raw data to ensure persistence
            newPoints = rawPoints
            print(">>> AppLocationManager: WASM Compression Fallback (Raw Points used: \(rawPoints.count))")
        }
        
        // 4. [REMOVED] Update UI trail here (Moved to didUpdateLocations for real-time responsiveness)
    ...
    private func savePointsToDatabase(_ points: [PathPoint]) {
        ...
        // [FIX] Use mainContext for guaranteed UI synchronization and simpler actor handling
        let context = AllToDoApp.sharedModelContainer.mainContext
        ...
        if currentTripID == nil {
            ...
            // [FIX] Set explicit time fields to pass UI filters (±24h)
            newTrip.begin_time = Date()
            newTrip.date_time = Date()
        ...
        do {
            try context.save()
            print(">>> Continuous Persistence: Successfully Saved \(points.count) pts to Trip \(tripID)")
        } catch {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if isRecording {
            ...
            // [NEW] Update UI trail immediately for real-time responsiveness
            DispatchQueue.main.async {
                self.processedSessionPoints.append(data)
            }
            
            pendingBuffer.append(data)
            
            // [FIX] Race Condition Guard: Prevent multiple simultaneous processBuffer calls
            if !isProcessingBuffer {
                if currentTripID == nil || pendingBuffer.count >= 5 {
