...
- **수정 금지 사유**: 
    - 이 로직은 각 SDK의 서로 다른 좌표계와 이미지 처리 방식으로 동기화한 결과물입니다. 
    - **절대 지도 엔진 전체를 한 번에 리팩토링하지 마십시오.** 작은 오차만으로도 핀이 공중에 떠 보이거나 물풍선이 핀을 가리는 등의 시각적 파손이 발생합니다.

## 5. New Todo Visibility (신규 할 일 핀 노출 보장)
- **위치**: `ContentView.swift` (handleLongTap)
- **키워드**: `no_of_path: 1`
- **설명**: 지도를 터치하여 할 일을 새로 만들 때, 이동 경로가 없더라도 지도 필터링(no_of_path > 0)을 통과하도록 **실수값을 1로 강제**하는 로직입니다.
- **수정 금지 사유**: 
    - 이 값을 0으로 바꾸면 신규 등록된 할 일이 지도에서 즉시 사라지는 치명적인 UX 저하가 발생합니다.
    - **절대 이 값을 0으로 되돌리지 마십시오.** 사용자가 수동으로 찾은 필터링 우회 로직입니다.
