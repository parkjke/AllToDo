...
*   **아이콘**: ⭐ **별 (Star)** - 기록된 장소 표시 (필수).
*   **특이사항**: 경로 지도(Path) 등에서는 디버깅용 텍스트(Start/End)가 오버레이될 수 있음.
*   **상태**: 정적 (변화 없음).
...
핀이 뭉쳐 있을 때(Cluster) 대표 아이콘 표시 우선순위:
1.  **1순위**: **현재 위치 (Current Location)**. 그룹 내에 현재 위치가 포함되면 무조건 내 위치 아이콘 표시.
2.  **2순위**: **History** (개수 과반수 이상 시).
3.  **3순위**: **ToDo** (기본).
