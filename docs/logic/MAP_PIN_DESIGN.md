...
*   **아이콘**: 방패 모양 핀 + 내부 마크 (별, 사람 등 구분).
    3.  **위치 저장 (Location Bookmark - Type 03)**: 사용자가 지도상에 북마크한 지점. (Blue Star 마크 적용)
...
*   **크기 표준화 (Size Standardization)**:
    ...
    *   **Anchor Point**: **(0.392, 1.0)** - 캔버스 확장에 따른 중심점 보정 (핀 끝이 정확한 좌표를 가리키도록).
...
### 7.3. 특수 핀 정의 (Type 25 - Cancel/Bomb)
*   **용도**: 작업 취소 (Cancel) 또는 실패/폭파 상태 표현.
*   **디자인**:
    *   **형태**: 폭발하는 폭탄 모양 (Exploded Shape).
    *   **색상**: 배경 **Red (#FF3B30)**, 코어 **Orange (#FF9500)**.
    *   **특징**: 일반적인 원형/방패 모양이 아닌 비정형(Jagged) 디자인으로 강조됨.
