...
### 5.3. 무결성 검증 (Parity Check)
*   **안전한 저장**: 파일 저장 시 단순 이미지가 아닌, 커스텀 헤더(`ALLTODO_V1`)를 포함하여 저장합니다.
*   **검증 로직**: 파일을 불러올 때 헤더를 체크하여 깨진 파일이거나 버전이 다르면 즉시 폐기하고 원본 SVG로부터 재생성합니다.
...
### 7.1. iOS 스케일링 기준
*   **기준 사이즈 (Standard)**: **40pt x 50pt** (Apple Map 기준)
*   **애플 맵 (Apple Map)**: **1.0x** (40x50pt)
*   **구글 맵 (Google Map)**: **1.0x** (40x50pt)
*   **네이버 맵 (Naver Map)**: **0.9x** (36x45pt) - 지도 정보 밀도를 고려한 최적화.
*   **카카오 맵 (Kakao Map)**: **0.75x** (30x37.5pt) -> **0.7x (28x35pt)**로 최종 조정 (사용자 가시성 선호도 반영).

### 7.2. Android 스케일링 기준
*   **기본 크기**: **40dp x 50dp** (모든 지도 제공자 공통 적용).
*   **앵커 (Anchor)**: **(0.4, 1.0)** - iOS 스타일의 우상단 뱃지 오버행(Overhang) 여백을 고려한 핀 끝점 정렬.
