...
- **클러스터링 전략**:
    ...
    - **Smoothing**: **4단계 전환 알고리즘** (신규 진입 -> 병합 정리 -> 구 클러스터 삭제 -> 신규 클러스터 추가) 적용으로 부드러운 핀 전환 구현.
    - **Guarding**: **NaN 좌표 가드**를 통해 지도 엔진 안정성 확보.
- **원거리 필터 (500km Filter)**:
    - **기준**: 현재 위치에서 **500km**를 초과하는 핀은 지도 표시 및 계산에서 제외.
    - **알림**: 제외된 핀이 있을 경우 "현재 위치에서 너무 먼 핀 X개는 표시되지 않습니다." 토스트 노출.
...
## 2. 앱 실행 및 포그라운드 전환 시퀀스 (3-Stage Sequence)
- **트리거(Trigger)**: 
  1. 앱 콜드 런치 (Cold Launch)
  2. 백그라운드 진입 후 **5초 이상** 경과 후 포그라운드 복귀 (Warm Launch > 5s)
...
*   **동작**:
    1.  **기본 위치**: **직전 위치(Before Location)** 또는 광화문을 중심으로 표시.
