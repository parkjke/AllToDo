...
- **크기**: 화면 하단 70%(Android 0.7f / iOS 550pt)를 점유하는 바텀 시트 스타일.
- **미니멀 디자인 (v2.0)**:
    - **라벨 제거**: 기존의 외부 텍스트 라벨을 삭제하고 입력창 내부 **플레이스홀더(Placeholder)**로 통합하여 시각적 복잡도를 제거.
    - **가독성 (iOS)**: 폰트 크기를 **19pt**로 상향하여 가이드 문구와 입력값의 시인성을 극대화.
- **비주얼 표준**:
    - Placeholder: **Gray 7 (`#616161`)**
    - 입력 텍스트: **Gray 9 (`#212121`)**
    - 배경: White (#FFFFFF) 및 RoundedCorner (24dp)
- **사용자 경험**:
    - **메모 공간 최적화**: 창 전체 높이는 유지하되, 메모 필드의 기본 높이(iOS 180pt)를 확장하여 쾌적한 기록 환경 제공.
...
- **표시 필터**: 지도의 핀은 **어제(-24시간)부터 내일 이후(무제한)**의 모든 데이터를 노출하여 탐색의 연속성을 보장합니다.
